...
for llt in latent_var:
    epochs = 10
    distrib_type = 'g'
    mmd_typ = ['mmd', 'mah', 'mah_gcvae']
    latent_dims = llt
    lls = ['VAE', r'$\beta$-VAE', 'InfoVAE', r'GCVAE$^\dagger$', r'GCVAE$^\ddagger$']
    ...
    for w, (i, j) in zip(range(5), logger.items()):
        for q, (n, m) in zip(range(5),  j.items()):
    ...
    # # #%%
    fig, ax = plt.subplots(1, len(met.keys()), subplot_kw={"projection": "3d"})
    fig.subplots_adjust(hspace = .5, wspace = .001)
    ax = ax.ravel()
    
    for en, (i, j) in enumerate(met.items()):
        # ax = plt.axes(projection='3d')
        ax[en].scatter(j['z_ts'][:, 0], j['z_ts'][:, 1], j['z_ts'][:, 0] , c = y_test, cmap='rainbow', linewidth = 0.5, s = 5)
        ax[en].set_xlabel(r'$z_1$')
        ax[en].set_ylabel(r'$z_2$')
        ax[en].set_zlabel(r'$z_3$')
        ax[en].set_title(f'{i}')
    
    
    #%%
    for md, lat_val in met.items():
        print('-'*120)
        print(f'|\t\t\t {md} \t\t\t|')
        print('-'*120)
        for clf in classifiers:
            clf.fit(lat_val['z_tr'], y_train)
            ...
            print('****Results****')
            train_predictions = clf.predict(lat_val['z_ts'])
            ...
            train_predictions = clf.predict_proba(lat_val['z_ts'])
